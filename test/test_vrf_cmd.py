...
LastEditTime: 2022-04-23 17:22:41
...
def test_decode_cmd4c_temp():
    ...
    myprint_cmd4c(dec_cmd4c.data_decode_dict)
...
def test_decode_cmd0D_resmain():
    ...
    for i in range(len(datas)):
        ...
        dec_cmd0D.cmd_check()
        myprint_cmdRES0D(dec_cmd0D.data_decode_rescmd0d)
        logging.debug('----------------------------')

def test_decode_cmd0D_rc():
    ...
    for i in range(len(datas)):
        dec_cmd0D = RD505CMD(datas[i])
        dec_cmd0D.cmd_check()
        myprint_cmd0D_rc(dec_cmd0D.data_decode0d_rc_dict)
        logging.debug('----------------------------')
